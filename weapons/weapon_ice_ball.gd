class_name WeaponIceBall
extends WeaponBase

var ball_count = 1

func _ready():
	super._ready()
	weapon_name = "Buz Topu"
	tag = "buyu"
	category = "defense"
	damage = 18
	cooldown = 2.2

func has_targets_for_attack() -> bool:
	return _any_enemy_within_distance(GameplayConstants.ICE_BALL_ENGAGE_RANGE_PX * player.get_area_multiplier())

func attack():
	var enemies = EnemyRegistry.get_enemies()
	if enemies.is_empty():
		return
	enemies.sort_custom(func(a, b):
		return player.global_position.distance_to(a.global_position) < player.global_position.distance_to(b.global_position)
	)
	for i in min(ball_count, enemies.size()):
		var atk: Dictionary = player.roll_attack_damage(damage)
		var ball = ObjectPool.get_object("res://projectiles/ice_ball.tscn")
		ball.global_position = player.global_position
		var dir = (enemies[i].global_position - player.global_position).normalized()
		ball.init(dir, atk.damage, player, atk.crit)

func on_upgrade():
	match level:
		2: damage = 24; cooldown = 2.0
		3: ball_count = 2; damage = 30; cooldown = 2.0
		4: damage = 36; cooldown = 1.8
		5: ball_count = 3; damage = 45; cooldown = 1.5

func get_description() -> String:
	return tr("ui.upgrade_ui.stats.loadout_weapons.ice_ball") % [
		level,
		ball_count,
		damage,
		snappedf(get_effective_cooldown(), 0.01),
	]
