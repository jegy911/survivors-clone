...
| 2026-04-23 | **Run HUD envanter + map_select lanet UI + ses dip’i** — `PlayerUiHelpers.rebuild_run_loadout_hud` (ikon + `LVL`, `UpgradeIconCatalog`); `map_select` koşu laneti açıklama/çubuklar + `ui/run_curse_stat_bar.gd` + `SaveManager.RUN_CURSE_*` / `tr_en_source`; Ayarlar → Ses `combat_music_duck` + `AudioManager.notify_combat_music_duck_beat`; `GELISTIRICI` / `TASARIM` / `sesler-muzikler-efektler` / matris / `survivors_clone_context` güncellendi. |
